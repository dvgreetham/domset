...
def cost_fun(G, nodes, doms, domd):
    ...
    for node in nodes:
        ...
        for u in G.neighbors(node):
            if u in doms or u in domd:
                 l+=1
            
        cf=ww/max(1, (G.degree(node)-l))
...
def greedy2(alpha, file, path):
    ...
    while r>0:
        ...
        #print(len(doms), len(nodes), len(domd))
    ...
    if is_dominated(G, G.nodes(), doms, domd, alpha)>0:
        print('Violations!!')
...
if __name__ == "__main__":
    ...
    path = os.path.join(current_dir, "Greedy1-new13.csv")
    print(path)
